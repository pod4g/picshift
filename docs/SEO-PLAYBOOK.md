...
- 使用连字符分隔 — 已做
- 全小写 — 已做
- **尾斜杠与规范 URL 全站一致** — 见下文「尾斜杠与规范 URL」：`rel="canonical"`、主要内链、`llms.txt` 中的 URL 应与用户与爬虫最终落地的 URL 一致（PicShift 的英文 **blog** 以**带尾斜杠**为规范）
...
### 尾斜杠与规范 URL（经验教训，2026-04-11）

#### 搜索引擎更在意什么

- Google **不**因为「带尾斜杠」或「不带尾斜杠」本身给更高排名；在意的是：**全站只保留一种可索引的规范形式**，另一种用 **301 或 308** 永久合并（至少不要长期两套 URL 都 200 且互不跳转）。
- 实践中任选一种即可；**必须与静态托管/CDN 的最终落地 URL 一致**，否则 `canonical`、结构化数据里的 `url` 与真实跳转链对不上，浪费抓取预算且信号混乱。

#### PicShift 线上实际行为（Cloudflare Workers 静态资源）

- 文章类页面构建为目录形态（如 `blog/what-is-exif-data/index.html`）时，访问 **无尾斜杠**路径（如 `/blog/what-is-exif-data`）常出现 **307** 跳转到 **有尾斜杠**（`/blog/what-is-exif-data/`）。
- **307 不等于错误**：这是边缘把请求归并到「目录」规范 URL 的常见做法；**不必**为消灭 307 去关 Cloudflare 功能。若追求语义上的「永久归并」，可在边缘改为 **301/308**（可选，收益边际）。
- **应做的**：把 **`rel="canonical"`、`og:url`、Article/Breadcrumb 等 JSON-LD 中的文章 URL、sitemap 中的 `<loc>`、站内 `<a href>`、`llms.txt` / `llms-full.txt` 的 Key pages** 全部写成与 **最终 200 页面**相同的形态（本站英文 **blog** 统一为 **带尾斜杠**）。
- **GSC**：「请求编入索引」、对外分享链接时，使用与规范一致的 URL（避免先无斜杠再 307）。

#### 与站内其他约定

- 工具页、多语言路径在 sitemap 中本就大量使用尾斜杠；blog 与列表页 `/blog/` 与之对齐，减少「同一内容两种写法」的统计分叉（与 GSC 里 `/pt/jpg-to-png` 与 `/pt/jpg-to-png/` 类问题同理：**规范 URL 只能选一个**）。
...
3. **canonical 在 `tools.ts`、英文展示在 `en.ts` 时要双写对齐**：否则审计过了、线上英文 meta 仍是旧文案。
4. **尾斜杠策略要「构建 + meta + 内链 + hreflang」四处一致**：仅改 `canonical` 不改 `localePath` 或 Astro `trailingSlash`，仍会出现用户点进无斜杠 URL 再 307 的体验与信号噪声；**hreflang** 若漏归一，等于主动告诉搜索引擎一组不一致的备用 URL。本轮用 **`htmlCanonicalUrl` 单点归一** + **`trailingSlash: 'always'`** 兜底。
